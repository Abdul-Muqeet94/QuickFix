(function () {
  ...
  app.controller("calenderController", function ($scope, $http, ModalService) {
    ...
    $scope.alertOnDrop = function (event, jsEvent, view) {
      for (var i = 0; i < $scope.events.length; i++) {
        if ($scope.events[i].id == event.id) {
          ...
          $scope.message = $http.post('http://184.168.146.148:88/api/shift/delete', event.id).
            then(function (response) {
              console.log(response.data);
    ...
    $scope.addEvent = function () {
      ...
      $scope.message = $http.post('http://184.168.146.148:88/api/shift/create/', $scope.new).
        then(function (response) {
          console.log(response.data);


          $scope.message = $http.post('http://184.168.146.148:88/api/shift/view/', '-1').
            then(function (response) {
